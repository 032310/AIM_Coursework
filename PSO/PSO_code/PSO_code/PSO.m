function PSO(FUN, DIM, ftarget, maxfunevals)
    ...
    popsize = 50;
    % Exploitation
    c1 = 1.4944; % 2 or 1.4944;
    % Exploration
    c2 = 1.4944; % 2 or 1.4944;
    % Inertia
    w = 0.792;
    xbound = 5;
    vbound = 5;
    ...
    gbest = pbest(index,:);
    maxfunevals = min(500000 * DIM, maxfunevals); %maxfunevals = min(10000, maxfunevals);
    maxiterations = ceil(maxfunevals/popsize);
    for iter = 2 : maxiterations
        % Update inertia weight
        % w = 0.9 - 0.8*(iter-2)/(maxiterations-2); % can comment out
        ...
        s = repmat(s',1,DIM);
        pbest = (1-s).*pbest + s.*x; [cost,index] = min(cost_p);
        gbest = pbest(index,:);
