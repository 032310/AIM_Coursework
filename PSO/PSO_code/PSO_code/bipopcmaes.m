...
defopts.Seed = '0  % evaluated if it is a string';
...
while irun <= nrestarts || sum(budget.smallpopsi) < sum(budget.largepopsi)  % for-loop does not work with resume
    ...
    if ~flgresume % not resuming a former run
        ...
        if 1 < 3 && irun > 2 && 1 * sum(budget.smallpopsi) < sum(budget.largepopsi)
            ...
            fgeneric(...
                'restart', ...
                sprintf('Restart because of %s: starting %d run with small popsize=%d', ...
                strflag, ...
                nrunswithsmallpopsize, lambda0));
        else
            ...
            if irun > 1,
                ...
                fgeneric(...
                    'restart', ...
                    sprintf('Restart because of %s: starting %d run with large popsize=%d', ...
                    strflag, ...
                    irun-nrunswithsmallpopsize, lambda0));
            end
        end
...
runs = [nrunswithsmallpopsize, irun];
